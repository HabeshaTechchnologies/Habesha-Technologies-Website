        ...
        const n = () => {
          ...
          , l = () => {
            ...
            e.slides.forEach((t => {
                t.querySelectorAll(".swiper-slicer-image-clone").forEach(( (t, s) => {
                    ...
                    t.style.top = 100 / e.params.slicerEffect.split * s + "%",
                    i.style.top = `-${100 * s}%`) : (t.style.width = 100 / e.params.slicerEffect.split + "%",
                    t.style.left = 100 / e.params.slicerEffect.split * s + "%",
                    i.style.left = `-${100 * s}%`)
                }
                ))
            }
          ...
          , a = t => {
            e.slides.forEach((s => {
                ...
                i.forEach(( (s, n) => {
                    0 === t ? (s.style.transitionTimingFunction = "ease-out",
                    s.style.transitionDuration = r ? "0ms" : `${e.params.speed + e.params.speed / (i.length - 1) * (i.length - n - 1)}ms`) : (s.style.transitionTimingFunction = "",
                    s.style.transitionDuration = `${t + t / (i.length - 1) * (i.length - n - 1)}ms`)
                }
                ))
            }
            )),
            function({duration: t}) {
                const {slides: s, activeIndex: i} = e;
                if (0 !== t) {
                    let t = !1;
                    ...
                    const n = s => {
                        if (s.target !== r)
                            return;
                        if (r.removeEventListener("transitionend", n),
                        t)
                            return;
                        if (!e || e.destroyed)
                            return;
        ...
        s("setTranslate", ( () => {
            "slicer" === e.params.effect && ( () => {
                ...
                e.slides.forEach(( (s, i) => {
                    ...
                    const r = s.progress
                      , n = s.querySelector(".swiper-slide-content");
                    n && (n.style.transform = `translate${t}(${e.size * -r * 1.2}px)`),
                    s.querySelectorAll(".swiper-slicer-image-clone").forEach((e => {
                        const s = -r;
